...
import { ArrowLeft, Brain, Lock, AlertTriangle, Heart, CheckCircle2, Sparkles } from 'lucide-react';
...
export default function AIHealthAnalysis({ onNavigate }: AIHealthAnalysisProps) {
  ...
  const loadReport = async (forceRegenerate = false, recordIdOverride?: string | null) => {
    if (!user || accountType !== 'premium') return;

    setLoadingReport(true);
    setError(null);

    const recordId = analysisMode === 'current-record' ? (recordIdOverride ?? selectedRecordId) : undefined;
    const sourceQuery = analysisMode === 'current-record'
      ? supabase.from('ai_analyses').select('summary,risk').eq('user_id', user.id).eq('medical_record_id', recordId ?? '__none__').order('generated_at', { ascending: false }).limit(1)
      : supabase.from('ai_analyses').select('summary,risk').eq('user_id', user.id).is('medical_record_id', null).order('generated_at', { ascending: false }).limit(1);
  ...
  const handleDownloadSummary = () => {
    const lines = [
      report.headline,
      '',
      `Mode: ${analysisMode === 'current-record' ? 'Current Record Analysis' : 'Complete Analysis'}`,
      `Risk: ${report.risk}`,
      '',
      'Summary',
      report.summary,
      '',
      'Key Findings',
      ...(report.keyFindings.length ? report.keyFindings.map((item, idx) => `${idx + 1}. ${item}`) : ['No findings available.']),
      '',
      'Red Flags',
      ...(report.redFlags.length ? report.redFlags.map((item, idx) => `${idx + 1}. ${item}`) : ['No immediate red flags found.']),
      '',
      'Next Steps',
      ...(report.nextSteps.length ? report.nextSteps.map((item, idx) => `${idx + 1}. ${item}`) : ['No next steps available.']),
    ];
    ...
    anchor.href = url;
    anchor.download = `medvault-summary-${analysisMode}-${Date.now()}.txt`;
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                ...
                {records.map((record) => (
                  ...
                  >
                    <div className="text-sm font-bold text-slate-900">{record.disease || 'Medical Report'}</div>
                    <div className="text-xs text-slate-600 break-all line-clamp-2">{record.file_name}</div>
