...
const Assistant = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      if (data && data.length > 0) {
        ...
        setMessages([
          { id: 'greeting-user', role: 'user', content: greeting.message, created_at: greeting.created_at },
          { id: 'greeting-ai', role: 'ai', content: greeting.ai_response, created_at: greeting.created_at }
        ]);
  ...
  const handleSend = async (e: React.FormEvent | string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: `ai-${Date.now()}`, role: 'ai', content: aiResponseText }]);

      await addChatExchange({
        user_id: user.id,
        message: text,
        ai_response: aiResponseText,
        context_summary: createContextSummary(text, aiResponseText) || contextSummarySeed
      });
