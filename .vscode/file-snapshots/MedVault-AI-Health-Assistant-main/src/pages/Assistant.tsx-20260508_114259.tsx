...
import { Send, Bot, User, Sparkles, Image as ImageIcon, Languages } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { useAuth } from '../contexts/useAuth';
import { getChatHistory, addChatExchange } from '../services/api';
import { supabase } from '../services/supabase';
import { buildAssistantContext } from '../services/ai/contextBuilder';
import { generateGeminiContent, ensureDisclaimer, AI_DISCLAIMER } from '../services/ai/gemini.service';
...
const Assistant = () => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      if (data && data.length > 0) {
        ...
        const greeting = {
          user_id: user.id,
          message: "Hello! I'm your CliniNova AI Assistant. How can I help you understand your health records today?",
          ai_response: ensureDisclaimer(
            'I can help summarize your reports, explain terminology, and provide multilingual insights.'
          ),
          created_at: new Date().toISOString()
        };
        await addChatExchange(greeting);
      }
      setFetching(false);
    };
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_history', filter: `user_id=eq.${user.id}` }, payload => {
        setMessages(prev => {
          ...
          if (row.message) {
            ...
            if (!next.find(m => m.id === id)) next.push({ id, role: 'user', content: row.message, created_at: row.created_at });
          }
          if (row.ai_response) {
            const id = `${row.id}-ai`;
            if (!next.find(m => m.id === id)) next.push({ id, role: 'ai', content: row.ai_response, created_at: row.created_at });
          }
  ...
  const handleSend = async (e: React.FormEvent | string) => {
    ...
    try {
      const { userPrompt, contextSummarySeed } = await buildAssistantContext({
        userId: user.id,
        userMessage: text,
        language
      });
      ...
      await addChatExchange({
        ...
        context_summary: contextSummarySeed
      });
